...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      walletInfo = { alias: "MockWallet", methods: ["lookup_invoice"] }
      log('📱 Wallet info:', {
      ...
      if (walletInfo.methods && Array.isArray(walletInfo.methods)) {
        ...
        if (!hasLookupInvoice) {
          ...
          return NextResponse.json({
            success: false,
            paid: false,
            error: 'NWC connection does not have lookup_invoice permission.',
    ...
    try {
      ...
      try {
        ...
        // MOCK: Check if paymentHash contains "mock"
        const isMock = paymentHash.includes('mock') || invoiceString.includes('mock')

        invoiceStatus = {
          settled: isMock, // Auto-confirm mock invoices
          paid: isMock,
          state: isMock ? 'SETTLED' : 'OPEN',
          amount: 1000,
          settled_at: Date.now() / 1000
        }

        lookupMethod = 'nwc_invoice_direct'
        log('✅ Invoice lookup successful! (MOCK)')
